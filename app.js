...
async function init(){
  ...
  try{
    state.data = await fetch('data.json?v=preload-assets1').then(r=>{ if(!r.ok) throw new Error('data.json '+r.status); return r.json(); });
    render();
...
function collectLandingAssetUrls(){
  const urls = new Set();
  extractAssetUrlsFromData(state.data, urls);
  $$('img[src]').forEach(img=>urls.add(img.getAttribute('src')));
  $$('video[src]').forEach(video=>urls.add(video.getAttribute('src')));
  $$('video[poster]').forEach(video=>urls.add(video.getAttribute('poster')));
  $$('link[rel="preload"][href]').forEach(link=>{
    const href=link.getAttribute('href');
    if(/^assets\//.test(href)) urls.add(href);
  });
...
function preloadOneAsset(url){
  ...
  const timeout = wait(15000).then(()=>({url, status:'timeout'}));
  let task;
  if(core.endsWith('.mp4')){
    task = fetch(src, {cache:'force-cache'}).then(r=>r.ok ? r.blob() : null).then(()=>({url,status:'ok'})).catch(()=>({url,status:'error'}));
  }else{
